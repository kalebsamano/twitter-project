    ...
    with open(file_name, 'r') as f:
        for line in f:
            ...
            if line != '\n':
                ...
                if len(rows) == 0:
                    cursor.execute('''
                        INSERT INTO USERS (id, verified, followers_count,
                            ...
                            VALUES
                                (?,?,?,?,?,?)
                    ''', (userID, verified, followers_count, name, handle,
                            location))
                    conn.commit()
                #insert place object
                ...
                if len(rows) == 0:
                    cursor.execute('''
                        INSERT INTO TWEETS (id, tweet_text, tweet_user,
                            favorite_count, created_at, lang, retweet_count,
                            reply_count, place_id, search_id)
                            VALUES
                                (?,?,?,?,?,?,?,?,?,?)
                    ''', (tweet_id, tweet_text, userID, favorite_count,
                            created_at, lang, retweet_count, reply_count,
                            place_id, searchID))
                    conn.commit()
                # insert hashtags
